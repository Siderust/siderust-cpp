...
template <typename T>
struct spherical_direction_frame; // undefined primary
...
class Target : public Trackable {
  ...
  public:
    ...
    explicit Target(C dir, JulianDate epoch = JulianDate::J2000())
        ...
        check_status(
            siderust_target_create(
                m_icrs_.ra().value(), m_icrs_.dec().value(), epoch.value(), &h),
            "Target::Target");
        handle_ = h;
    }
    ...
    qtty::Degree ra() const { return m_dir_.ra(); }

    /// Declination — only available for equatorial frames (RA/Dec).
    template <typename F_ = Frame, std::enable_if_t<frames::has_ra_dec_v<F_>, int> = 0>
    qtty::Degree dec() const { return m_dir_.dec(); }
...
using EquatorialMeanJ2000Target = Target<spherical::direction::EquatorialMeanJ2000>;

/// Fixed direction in mean equatorial coordinates of date (precessed only).
using EquatorialMeanOfDateTarget = Target<spherical::direction::EquatorialMeanOfDate>;

/// Fixed direction in true equatorial coordinates of date (precessed + nutated).
using EquatorialTrueOfDateTarget = Target<spherical::direction::EquatorialTrueOfDate>;
