...
inline qtty::Radian altitude_at(Body b, const Geodetic& obs, const MJD& mjd) {
    double out;
    check_status(siderust_body_altitude_at(
                     static_cast<SiderustBody>(b), obs.to_c(), mjd.value(), &out),
                 "body::altitude_at");
...
inline std::vector<CrossingEvent> crossings(
    ...
    uintptr_t                  count = 0;
    check_status(siderust_body_crossings(
                     static_cast<SiderustBody>(b), obs.to_c(),
                     window.c_inner(), threshold.value(),
                     opts.to_c(), &ptr, &count),
                 "body::crossings");
...
inline std::vector<CulminationEvent> culminations(
    ...
    uintptr_t                     count = 0;
    check_status(siderust_body_culminations(
                     static_cast<SiderustBody>(b), obs.to_c(),
                     window.c_inner(),
                     opts.to_c(), &ptr, &count),
                 "body::culminations");
...
inline std::vector<Period> altitude_periods(
    ...
    tempoch_period_mjd_t*     ptr   = nullptr;
    uintptr_t                 count = 0;
    check_status(siderust_body_altitude_periods(
                     static_cast<SiderustBody>(b), q, &ptr, &count),
...
inline qtty::Radian azimuth_at(Body b, const Geodetic& obs, const MJD& mjd) {
    double out;
    check_status(siderust_body_azimuth_at(
                     static_cast<SiderustBody>(b), obs.to_c(), mjd.value(), &out),
                 "body::azimuth_at");
    return qtty::Radian(out);
}
...
inline std::vector<AzimuthExtremum> azimuth_extrema(
    ...
    uintptr_t                    count = 0;
    check_status(siderust_body_azimuth_extrema(
                     static_cast<SiderustBody>(b), obs.to_c(),
                     window.c_inner(),
                     opts.to_c(), &ptr, &count),
...
inline std::vector<Period> in_azimuth_range(
    Body b, const Geodetic& obs, const Period& window,
    qtty::Degree min, qtty::Degree max, const SearchOptions& opts = {}) {
    tempoch_period_mjd_t* ptr   = nullptr;
    uintptr_t             count = 0;
    check_status(siderust_body_in_azimuth_range(
                     static_cast<SiderustBody>(b), obs.to_c(),
                     window.c_inner(), min.value(), max.value(),
                     opts.to_c(), &ptr, &count),
