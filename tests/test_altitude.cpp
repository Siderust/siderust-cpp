...
TEST_F(AltitudeTest, SunAltitudePeriods) {
  ...
  auto periods =
      sun::altitude_periods(obs, window, qtty::Degree(-6.0), qtty::Degree(0.0));
  for (auto &p : periods) {
    EXPECT_GT(p.duration().value(), 0.0);
  }
}
...
TEST_F(AltitudeTest, StarAboveThreshold) {
  const auto &vega = VEGA;
  auto periods =
      star_altitude::above_threshold(vega, obs, window, qtty::Degree(30.0));
...
TEST_F(AltitudeTest, IcrsAltitudeAt) {
  const spherical::direction::ICRS vega_icrs(qtty::Degree(279.23),
                                             qtty::Degree(38.78));
