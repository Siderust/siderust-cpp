...
Orbit satellite_orbit() {
  return {0.0001_au, 0.0, 0.0_deg, 0.0_deg, 0.0_deg, 0.0_deg, J2000};
}

// Approximate Mars heliocentric orbit
Orbit mars_orbit() {
  return {1.524_au, 0.0934, 1.85_deg, 49.56_deg, 286.5_deg, 19.41_deg, J2000};
}
...
TEST(BodycentricTransforms, GeocentricToBodycentricGeoOrbit) {
  ...
  cartesian::Position<Geocentric, EclipticMeanJ2000, AstronomicalUnit> target(
      0.001, 0.0, 0.0);

  auto result = to_bodycentric(target, params, jd);

  // Satellite is at ~0.0001 AU; relative position should be positive and <
  // 0.001 AU
  ...
  EXPECT_NEAR(result.center_params().orbit.semi_major_axis.value(), 0.0001,
              1e-10);
}
...
TEST(BodycentricTransforms, HeliocentricToBodycentricHelioOrbit) {
  ...
  double r =
      vec_magnitude(result.x().value(), result.y().value(), result.z().value());
  EXPECT_GT(r, 0.3);
  EXPECT_LT(r, 3.0);
...
TEST(BodycentricTransforms, RoundTripGeocentricBodycentric) {
  ...
  cartesian::Position<Geocentric, EclipticMeanJ2000, AstronomicalUnit> original(
      0.001, 0.002, 0.003);
...
TEST(BodycentricTransforms, BodyOwnPositionAtOrigin) {
  ...
  double r =
      vec_magnitude(body_from_body.x().value(), body_from_body.y().value(),
...
TEST(BodycentricTransforms, HeliocentricOrbitWithGeocentricOrbit) {
  ...
  // Moon at ~384400 km, ISS at ~6378 km → distance should be close to moon
  // distance
...
TEST(BodycentricFFI, NullOutputPointer_ToBodycentric) {
  siderust_cartesian_pos_t pos{1.0, 0.0, 0.0,
...
TEST(BodycentricFFI, NullOutputPointer_FromBodycentric) {
  siderust_cartesian_pos_t pos{0.5, 0.0, 0.0,
                               SIDERUST_FRAME_T_ECLIPTIC_MEAN_J2000,
...
TEST(BodycentricFFI, InvalidCenterInput) {
  siderust_cartesian_pos_t pos{
      1.0, 0.0, 0.0, SIDERUST_FRAME_T_ECLIPTIC_MEAN_J2000,
...
TEST(BodycentricTransforms, VenusHeliocentricIsFinite) {
  ...
  double r =
      vec_magnitude(venus.x().value(), venus.y().value(), venus.z().value());
