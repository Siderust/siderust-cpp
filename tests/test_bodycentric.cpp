...
TEST(BodycentricTransforms, GeocentricToBodycentricGeoOrbit) {
  ...
  cartesian::Position<Geocentric, EclipticMeanJ2000, AstronomicalUnit> target(0.001, 0.0, 0.0);
...
TEST(BodycentricTransforms, HeliocentricToBodycentricHelioOrbit) {
  ...
  double r = vec_magnitude(result.x().value(), result.y().value(), result.z().value());
  EXPECT_GT(r, 0.3);
  EXPECT_LT(r, 3.0);
...
TEST(BodycentricTransforms, BodyOwnPositionAtOrigin) {
  ...
  double r = vec_magnitude(body_from_body.x().value(), body_from_body.y().value(),
...
TEST(BodycentricTransforms, HeliocentricOrbitWithGeocentricOrbit) {
  ...
  cartesian::Position<Geocentric, EclipticMeanJ2000, AstronomicalUnit> moon_geo(0.00257, 0.0, 0.0);
