...
int main() {
  ...
  auto above_min =
      star_altitude::above_threshold(target, observer, window, min_alt);
  auto below_max =
      star_altitude::below_threshold(target, observer, window, max_alt);
  ...
  auto azimuth_periods =
      star_altitude::in_azimuth_range(target, observer, window, min_az, max_az);
  ...
  for (size_t i = 0; i < observable.size(); ++i) {
    ...
    std::cout << "  " << (i + 1) << ". MJD " << std::fixed
              << std::setprecision(6) << observable[i].start() << " -> "
              << observable[i].end() << "  (" << std::setprecision(4) << hours
              << ")" << std::endl;
  }
