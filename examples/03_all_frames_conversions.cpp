...
///
/// Build & run: cmake --build build-local --target
/// 03_all_frame_conversions_example
...
void show_frame_conversion(const JulianDate &jd,
  ...
  std::cout << std::left << std::setw(24) << FrameTraits<F1>::name() << " -> "
            << std::setw(24) << FrameTraits<F2>::name() << " out=("
            << std::showpos << std::setprecision(9) << out << std::noshowpos
            << ")  roundtrip=" << std::scientific << std::setprecision(3) << err
            << std::fixed << std::endl;
}
...
int main() {
  ...
  show_frame_conversion<EquatorialMeanJ2000, EquatorialMeanJ2000>(jd,
                                                                  p_eq_j2000);
  show_frame_conversion<EquatorialMeanOfDate, EquatorialMeanOfDate>(jd,
                                                                    p_eq_mod);
  show_frame_conversion<EquatorialTrueOfDate, EquatorialTrueOfDate>(jd,
                                                                    p_eq_tod);
  ...
  show_frame_conversion<EquatorialMeanJ2000, EquatorialMeanOfDate>(jd,
                                                                   p_eq_j2000);
  show_frame_conversion<EquatorialMeanOfDate, EquatorialMeanJ2000>(jd,
                                                                   p_eq_mod);
  show_frame_conversion<EquatorialMeanOfDate, EquatorialTrueOfDate>(jd,
                                                                    p_eq_mod);
  show_frame_conversion<EquatorialTrueOfDate, EquatorialMeanOfDate>(jd,
                                                                    p_eq_tod);
  show_frame_conversion<EquatorialMeanJ2000, EquatorialTrueOfDate>(jd,
                                                                   p_eq_j2000);
  show_frame_conversion<EquatorialTrueOfDate, EquatorialMeanJ2000>(jd,
                                                                   p_eq_tod);
