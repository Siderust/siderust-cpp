...
static OrbitPoint hermite_interp(const Orbit &pts, double t) {
  ...
  std::size_t idx = 0;
  {
    ...
    idx = (lo == 0) ? 0 : lo - 1;
    if (idx >= pts.size() - 1) idx = pts.size() - 2;
    // Exact hit?
    if (pts[idx + 1].epoch_j2000_s == t) return pts[idx + 1];
    if (pts[idx].epoch_j2000_s == t) return pts[idx];
  }
  ...
  return {t,
          ip(p0.px, p0.vx, p1.px, p1.vx), ip(p0.py, p0.vy, p1.py, p1.vy),
          ip(p0.pz, p0.vz, p1.pz, p1.vz),
          iv(p0.px, p0.vx, p1.px, p1.vx), iv(p0.py, p0.vy, p1.py, p1.vy),
...
struct LisaEphemerisProvider {
  ...
  const Orbit &orbit_for(int naif) const {
    switch (naif) {
    case -1001: return orbits[0];
    case -1002: return orbits[1];
    case -1003: return orbits[2];
    default:    throw std::invalid_argument("unknown LISA NAIF id");
    }
  }
...
static double modelled_range_m(const LisaEphemerisProvider &prov,
                                int sc_a_naif, int sc_b_naif,
...
#define SIDERUST_TEST_DATA_DIR "siderust/tests/test-data/lisa"
  ...
  const double meas_m = modelled_range_m(prov,
                                          naif_id(LisaScId::SC1),
                                          naif_id(LisaScId::SC2),
                                          sc1_t0);
  ...
  const double computed_m = modelled_range_m(prov,
                                              naif_id(LisaScId::SC1),
                                              naif_id(LisaScId::SC2),
                                              sc1_t0);
