...
int main() {
  ...
  const KeplerianOrbit kepler{1.0_au,    0.0167,   0.0_deg,   0.0_deg,
                              102.9_deg, 100.0_deg, 2451545.0};
  const auto kepler_pos = kepler_position(kepler, jd);
  std::puts("1. KeplerianOrbit");
  std::printf("   heliocentric position = (%.12f, %.12f, %.12f) AU\n",
              kepler_pos.x().value(), kepler_pos.y().value(),
              kepler_pos.z().value());
  std::printf("   radius = %.12f AU\n\n", kepler_pos.distance().value());

  const MeanMotionOrbit mean_motion{1.0_au,   0.0167, 0.0_deg, 0.0_deg,
                                    102.9_deg, 0.9856, 2451545.0};
  const auto mean_motion_pos = mean_motion.position_at(jd);
  std::puts("2. MeanMotionOrbit");
  std::printf("   heliocentric position = (%.12f, %.12f, %.12f) AU\n",
              mean_motion_pos.x().value(), mean_motion_pos.y().value(),
  ...
  const ConicOrbit hyperbolic{0.255_au, 1.2,       122.7_deg, 24.6_deg,
                              241.8_deg, 12.0_deg, 2451545.0};
  ...
  std::printf("   halley-like kind = %s\n",
              halley_like.kind() == ConicKind::Elliptic ? "Elliptic"
                                                        : "Hyperbolic");
  std::printf("   hyperbolic kind  = %s\n",
              hyperbolic.kind() == ConicKind::Elliptic ? "Elliptic"
                                                       : "Hyperbolic");
  ...
  const double delta = std::sqrt(
      std::pow(kepler_pos.x().value() - prepared_pos.x().value(), 2) +
