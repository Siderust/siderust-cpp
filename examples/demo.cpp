...
int main() {
  ...
  targets.push_back(
      {"Fixed target", std::make_unique<ICRSTarget>(spherical::direction::ICRS{
  ...
  for (const auto &entry : targets) {
    ...
    std::cout << "  " << std::setw(12) << std::left << name
              << " alt=" << std::setw(8) << alt << " az=" << az << std::endl;
  }
  ...
  double moon_dist_km = std::sqrt(moon_geo.x().value() * moon_geo.x().value() +
  ...
  auto bright_periods =
      moon::illumination_above(Period(now, now + qtty::Day(7.0)), 0.8);
