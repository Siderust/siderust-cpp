...
void section_vsop87_positions(const JulianDate &jd) {
  ...
  std::cout << "Earth-Mars separation:       " << std::fixed
            ...
            << std::setprecision(0) << earth_mars.to<qtty::Kilometer>() << ")"
            << std::endl;
...
void section_trait_dispatch(const JulianDate &jd) {
  ...
  const PlanetEphem planets[] = {
      {"Mercury", &ephemeris::mercury_heliocentric,
  ...
  for (auto &[name, helio_fn, bary_fn] : planets) {
    ...
    std::cout << std::left << std::setw(8) << name << " helio=" << std::fixed
              << std::setprecision(5) << helio.distance()
...
void section_custom_planet() {
  ...
  Planet demo_world{qtty::Kilogram(5.972e24 * 2.0), // mass: double the Earth
                    ...
                    Orbit{1.4_au, 0.07, 4.0_deg, 120.0_deg, 80.0_deg, 10.0_deg,
